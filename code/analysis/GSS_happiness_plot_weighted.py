...
Purpose: Design-based, survey-weighted version of GSS_happiness_plot.py. Same 2004-2024
         ...
         code/analysis/ copy: subgroups within each category are ordered most to
         least happy by their true weighted mean, and output goes to output/analysis/.
"""
...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "output", "analysis", "GSS")
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "GSS_happiness_truth.png")
...
def build_results(df):
    ...
    for category, var, label_func in subgroup_specs:
        ...
        if not rows:
            continue
        # Order subgroups within the category most happy -> least happy by their
        # true weighted mean (this copy's difference from the descriptives version).
        summary = (
            pd.DataFrame(rows)
...
SUBTITLE = ("General Social Survey, 2004-2024; WTSSNRPS-weighted subgroup means with design-based "
            "95% CIs (VPSU/VSTRAT)")
