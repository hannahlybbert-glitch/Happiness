...
Purpose: Same plot as ingroup_outgroup_gap.py (own-subgroup minus other-subgroups
         PREDICTION gap, one red square per subgroup), with a second point per
         subgroup: the identical gap computed on the TRUE (WTSSNRPS-weighted) GSS
         happiness instead of the predictions -

             truth_gap(S) = true_mean(S) - mean_{O != S} true_mean(O)

         e.g. Gender / Man = male true mean - female true mean; Age / 65+ = 65+
         true mean - average of the 18-34 and 35-64 true means. These points are
         orange. Positive orange point = the subgroup really is happier than the
         rest of its group; positive red square = the subgroup PREDICTS its own
         group happier than the rest.

         Predictions are unweighted; predictor membership, the Race White/Black/
         Other pooling, and the group config are reused from
         ingroup_outgroup_predictions.py. Rows within each category are ordered by
         true GSS happiness, happiest first.

Outputs: output/analysis/ingroup_outgroup_gap_with_truth.png
         output/analysis/ingroup_outgroup_gap_with_truth.md
"""
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "ingroup_outgroup_gap_with_truth.png")
OUTPUT_TABLE_MD = os.path.join(OUTPUT_DIR, "ingroup_outgroup_gap_with_truth.md")

POINT_COLOR = "#D62728"  # red - prediction gap (own - other subgroups)
TRUTH_COLOR = "#E69F00"  # orange - true-happiness gap (own - other subgroups)

TITLE = "In-group vs. out-group happiness: predicted gap vs. true gap"
SUBTITLE = (
    "Red square: own subgroup − predicted mean of the other subgroups.  "
    "Orange: the same gap in true (GSS-weighted) happiness.  Rows ordered by true GSS happiness."
)
...
def compute_gap(category, cfg, actual_results, gss_df, qualtrics_df, crosswalk, sv_lookup):
    """One row per subgroup - subgroup, n_pred, own, other, gap, true_mean,
    truth_gap - sorted by true_mean (happiest first).

    gap       = own prediction of S  -  mean of S's members' predictions for each other subgroup
    truth_gap = true mean of S       -  mean of the true means of the other subgroups
    """
    ...
    for S in subgroups:
        ...
        other_truth = float(np.mean([true_mean[O] for O in subgroups if O != S]))
        recs.append({
            "subgroup": S, "n_pred": int(in_S.sum()),
            "own": own, "other": other, "gap": own - other,
            "true_mean": true_mean[S], "other_truth": other_truth,
            "truth_gap": true_mean[S] - other_truth,
        })
...
def plot_gap_results(results, output_path, title, subtitle):
    """Category-block layout with two points per subgroup: a red square at x = gap
    (prediction) and an orange circle at x = truth_gap (true GSS happiness), plus a
    dashed line at 0 and a red dashed line at the mean prediction gap."""
    apply_plot_style()
    ...
    ax_plot.text(gap_mean, y_hi, f" pred. mean = {gap_mean:+.3f}",
                 ha="left", va="top", fontsize=9, color=POINT_COLOR)

    ax_plot.plot(
        results["truth_gap"], results["y"], "o", color=TRUTH_COLOR,
        markeredgecolor=TRUTH_COLOR, markersize=6, linewidth=0, zorder=3,
    )
    ax_plot.plot(
        results["gap"], results["y"], "s", color=POINT_COLOR,
        markeredgecolor=POINT_COLOR, markersize=6, linewidth=0, zorder=4,
    )
    ...
    vals = pd.concat([results["gap"], results["truth_gap"]])
    lo, hi = min(0.0, float(vals.min())), max(0.0, float(vals.max()))
    pad = 0.08 * (hi - lo) if hi > lo else 0.05
    ax_plot.set_xlim(lo - pad, hi + pad)
    ax_plot.set_xlabel(
        "Own subgroup − mean of the other subgroups in the group"
    )
    ax_plot.legend(
        handles=[
            mlines.Line2D([], [], color=POINT_COLOR, marker="s", linestyle="", markersize=7,
                          markeredgecolor=POINT_COLOR, label="Prediction gap"),
            mlines.Line2D([], [], color=TRUTH_COLOR, marker="o", linestyle="", markersize=7,
                          markeredgecolor=TRUTH_COLOR, label="True-happiness gap"),
        ],
...
def write_gap_markdown(results, output_path, title, subtitle):
    lines = [
        f"# {title}", "", subtitle, "",
        "`Pred. gap` = `Own-group pred.` − `Mean other-group pred.` (average of that "
        "subgroup's members' mean predictions for each other subgroup). "
        "`Truth gap` = `True GSS mean` − `Mean other-group true` (average of the other "
        "subgroups' true GSS means). Positive `Pred. gap` = the subgroup *predicts* its "
        "own group happier than the rest; positive `Truth gap` = it really *is* happier. "
        "Rows are ordered by `True GSS mean`.",
        "",
        "| Category | Subgroup | N (predictors) | Own-group pred. | Mean other-group pred. | "
        "Pred. gap | True GSS mean | Mean other-group true | Truth gap |",
        "|---|---|---|---|---|---|---|---|---|",
    ]
    last_category = None
    for row in results.itertuples():
        ...
        lines.append(
            ...
            f"{row.other_truth:.3f} | {row.truth_gap:+.3f} |"
        )
...
def main():
    ...
    print(f"Wrote {len(results)} subgroup rows")
    print(f"  prediction gap: {results['gap'].min():+.3f} to {results['gap'].max():+.3f}"
          f"; mean {results['gap'].mean():+.3f}")
    print(f"  truth gap:      {results['truth_gap'].min():+.3f} to {results['truth_gap'].max():+.3f}"
          f"; mean {results['truth_gap'].mean():+.3f}")
