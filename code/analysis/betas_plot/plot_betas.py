...
Purpose: Plot the regression betas from run_betas_regression.R - effect of each
         ...
         code/analysis/ copy of code/descriptives/betas_plot/plot_betas.py: also splices
         the 14 categories into three slide-sized plots (same 5/5/4 grouping, shared
         x-scale, no title) the way code/analysis/GSS_happiness_plot_weighted.py splices
         GSS_happiness_truth.png.
"""
...
INPUT_CSV = os.path.join(PROJECT_ROOT, "data", "analysis", "betas_plot", "GSS_betas.csv")
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "output", "analysis", "betas_plot")
OUTPUT_PLOT = os.path.join(OUTPUT_DIR, "GSS_betas_plot.png")
OUTPUT_TABLE_MD = os.path.join(OUTPUT_DIR, "GSS_betas_table.md")

# Slide-sized spliced versions of the same plot: 14 categories split 5/5/4, no title,
# same x-axis scale - same grouping code/analysis/GSS_happiness_plot_weighted.py uses.
SPLICE_GROUPS = [
...
SPLICE_OUTPUT_FILES = [
    os.path.join(OUTPUT_DIR, f"GSS_betas_plot_part{i}.png") for i in range(1, len(SPLICE_GROUPS) + 1)
]
...
def plot_betas(results, output_path, title, subtitle, show_title=True, xlim=None):
    ...
    show_title=False omits the title/subtitle block entirely (for spliced sub-plots that
    share a single title elsewhere). xlim fixes the x-axis range explicitly (e.g. so a
    set of spliced plots all share the same scale instead of each autoscaling to its own
    ...
    if show_title:
        ...
        fig = plt.figure(figsize=(13.6, fig_height), constrained_layout=True)
        gs = fig.add_gridspec(1, 2, width_ratios=[2.0, 2.5], wspace=0.03)
...
def shared_xlim(results, pad_frac=0.05):
    """x-axis range covering every category's CI (plus the beta=0 reference line), with
    the same 5% padding matplotlib's default autoscale would add - so all spliced plots
    share one scale matching what the full, unsplit plot would show."""
    non_baseline = results[~results["is_baseline"]]
    lo = min(non_baseline["ci_lo"].min(), 0)
    hi = max(non_baseline["ci_hi"].max(), 0)
    pad = (hi - lo) * pad_frac
    return (lo - pad, hi + pad)


def plot_spliced(results):
    """Save the slide-sized spliced versions: same category groups as
    GSS_happiness_plot_weighted.py's splice, no title, shared x-scale, same LHS label
    formatting as the full plot."""
    xlim = shared_xlim(results)
    for categories, output_file in zip(SPLICE_GROUPS, SPLICE_OUTPUT_FILES):
        subset = results[results["category"].isin(categories)]
        plot_betas(subset, output_file, title=None, subtitle=None, show_title=False, xlim=xlim)
