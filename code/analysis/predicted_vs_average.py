...
Purpose: Main figures of difference between predicted and actual happiness scores
            Will also include some grass is greener and other analyses (as I can formulate them)
"""
...
OUTPUT_FILE = os.path.join(PROJECT_ROOT, "output", "analysis", "predicted_vs_actual_happiness.png")
...
def write_markdown_table(results, overall, output_path, title, subtitle):
    lines = [
        ...
        "N (predicted) | Predicted Mean | Predicted SE | Predicted 95% CI |",
        "|---|---|---|---|---|---|---|---|---|---|",
    ...
    for row in results.itertuples():
        ...
        lines.append(
            ...
            f"{row.n_predicted:,} | {row.mean_predicted:.3f} | {row.se_predicted:.3f} | "
            f"({row.ci_lo_predicted:.3f}, {row.ci_hi_predicted:.3f}) |"
        )
